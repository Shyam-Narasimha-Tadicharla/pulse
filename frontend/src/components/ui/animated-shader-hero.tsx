...
void main(){ gl_Position = position; }`);
    ...
    const onMove = (e: MouseEvent) => {
      ...
      stateRef.current.mouse = [e.clientX * dpr, canvas.height - e.clientY * dpr];
      stateRef.current.move = [e.movementX, e.movementY];
    };
    ...
    const loop = (now: number) => {
      ...
      if (!s) return;
      const { gl, program, uniforms, mouse, move } = s;
      ...
      gl.uniform2f(uniforms.move!, ...move as [number, number]);
      gl.uniform2f(uniforms.touch!, ...mouse as [number, number]);
